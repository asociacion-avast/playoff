...
for socio in socios:
    ...
    if socio.get("_valid_alta", False) and isinstance(socio["campsDinamics"], dict):
        for field in [common.fechacambio]:
            if field in socio["campsDinamics"]:
                ...
                if not (fecha and fecha <= today):
                    print(f"Fecha no alcanzada {fecha}: {common.sociobase}{socioid}")
                else:
